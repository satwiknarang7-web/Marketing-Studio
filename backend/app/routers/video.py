...
async def generate_video_endpoint(req: VideoGenerateRequest, db: AsyncSession = Depends(get_db)):
    try:
        video_url = await generate_video(req.prompt, req.duration_seconds, req.style, req.image_data)
        ...
        gen = Generation(
            ...
            type="video",
            prompt=req.prompt,
            result=json.dumps({"video_url": video_url}),
            model_used=settings.VIDEO_MODEL,
            style=req.style,
